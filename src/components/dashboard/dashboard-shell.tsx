      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            <p className="text-base font-semibold text-stone-900">{schedule.title}</p>
            <span className={`pill ${getScheduleStatusClasses(schedule.status)}`}>
              {SCHEDULE_STATUS_OPTIONS.find((item) => item.value === schedule.status)?.label}
            </span>
          </div>
          {showAssignee ? (
            <p className="text-sm text-stone-600">Assigned to {schedule.assignedUser.name}</p>
          ) : null}
          <p className="text-sm text-stone-500">{formatDateRange(schedule.startAt, schedule.endAt)}</p>
          {schedule.description ? (
            <p className="text-sm leading-6 text-stone-600">{schedule.description}</p>
          ) : null}
        </div>
          ...
          <div className="relative">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm text-stone-300">Operations dashboard</p>
              </div>
            ...
            <p className="mt-6 max-w-2xl text-lg leading-8 text-stone-200/85">
              Track your day, keep user access in order, and manage documents from
              a single secure interface.
            </p>

            <div className="mt-10 grid gap-4 sm:grid-cols-3">
              <div className="rounded-[1.4rem] border border-white/10 bg-white/8 p-4 backdrop-blur-sm">
                <p className="text-xs uppercase tracking-[0.24em] text-amber-100/80">Session</p>
                <p className="mt-3 text-base font-semibold text-white">{session.name}</p>
                <p className="mt-1 text-sm text-stone-200/80">{session.email}</p>
              </div>
              <div className="rounded-[1.4rem] border border-white/10 bg-white/8 p-4 backdrop-blur-sm">
                <p className="text-xs uppercase tracking-[0.24em] text-amber-100/80">Schedules</p>
                <p className="mt-3 text-3xl font-serif text-white">{schedules.length}</p>
                <p className="mt-1 text-sm text-stone-200/80">visible in your workspace</p>
              </div>
              <div className="rounded-[1.4rem] border border-white/10 bg-white/8 p-4 backdrop-blur-sm">
                <p className="text-xs uppercase tracking-[0.24em] text-amber-100/80">Access</p>
                <p className="mt-3 text-3xl font-serif text-white">{canManageSchedules ? 'Team' : 'Personal'}</p>
                <p className="mt-1 text-sm text-stone-200/80">
                  {canManageSchedules ? 'manage team operations' : 'see only your assigned shifts'}
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="panel flex flex-col justify-between gap-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-3 max-w-xl text-sm leading-6 text-stone-600">
                Navigate between your overview, schedules, documents, and audit tools.
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <div className="metric-card">
              ...
              <span className="metric-value">{canManageUsers ? activeUsersCount : 1}</span>
            </div>
            <div className="metric-card">
              <span className="metric-label">Audit Entries</span>
              <span className="metric-value">{canViewAudit ? auditLogs.length : 0}</span>
            </div>
          </div>
        </section>
      </div>
        ...
        <section className="space-y-6">
              ...
              <div className="panel">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <h3 className="mt-2 font-serif text-2xl text-stone-900">Upcoming schedules</h3>
                  </div>
                ...
                <div className="mt-6 space-y-3">
                  {upcomingSchedules.length > 0 ? (
                    ...
                    <p className="text-sm text-stone-500">No schedules available yet.</p>
                  )}
                </div>
              </div>
            </>
          ...
          {activeTab === 'Users' && canManageUsers ? (
            <>
              ...
              <div className="panel">
                ...
                <form onSubmit={handleUserSubmit} className="mt-6 space-y-4">
                  ...
                  <div className="flex flex-wrap justify-end gap-3">
                    <button type="button" onClick={cancelUserEdit} className="secondary-button">
                      Reset
                    </button>
              ...
              <div className="panel">
                <h3 className="font-serif text-2xl text-stone-900">Registered users</h3>
                <div className="mt-6 space-y-3">
                  {users.map((user) => (
                    <div
                      key={user.id}
                      ...
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div>
                          ...
                          <div className="mt-3 flex flex-wrap gap-2">
                            <span className="pill">
                              {ROLE_OPTIONS.find((role) => role.value === user.role)?.label}
                            </span>
                            <span
                              className={`pill ${
                                user.status === 'ACTIVE'
          ...
          {activeTab === 'Schedules' ? (
            <>
              <SectionIntro
                kicker="Schedules"
                title={canManageSchedules ? 'Plan work shifts' : 'Your schedule'}
                description={
                  canManageSchedules
                ...
                <div className="panel">
                  ...
                  <form onSubmit={handleScheduleSubmit} className="mt-6 space-y-4">
                    <div className="grid gap-4 md:grid-cols-2">
                      ...
                      <label className="block space-y-2">
                        <span className="text-sm font-medium text-stone-700">Start date and time</span>
                        <input
                          className="field"
                          type="datetime-local"
                      ...
                      <label className="block space-y-2">
                        <span className="text-sm font-medium text-stone-700">End date and time</span>
                        <input
                          className="field"
                          type="datetime-local"
                    ...
                    <div className="flex flex-wrap justify-end gap-3">
                      <button type="button" onClick={cancelScheduleEdit} className="secondary-button">
                        Reset
                      </button>
                      <button type="submit" disabled={busy} className="primary-button">
                ...
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <h3 className="mt-2 font-serif text-2xl text-stone-900">
                      {canManageSchedules ? 'Managed shifts' : 'Assigned shifts'}
                    </h3>
                  </div>
              ...
              <div className="panel">
                <div className="space-y-3">
                  {auditLogs.length > 0 ? (
                    auditLogs.map((log) => (
                      ...
                      >
                        <div className="flex items-start gap-4">
                          ...
                          <div>
                            <p className="text-sm font-medium text-stone-900">{log.description}</p>
                    ...
                    <p className="text-sm text-stone-500">No audit activity available yet.</p>
                  )}
                </div>
              </div>
            </>
